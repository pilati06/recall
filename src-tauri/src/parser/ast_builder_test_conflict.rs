...
use std::hash::{Hash, Hasher}; // Importar Hash

// --- 1. Definições da AST (COM 'Eq' E 'Hash' ADICIONADOS) ---
...
// Implementação manual de Hash para Clause (devido à recursão)
impl Hash for Clause {
    fn hash<H: Hasher>(&self, state: &mut H) {
        ...
        match self {
            Clause::True => {}
            Clause::False => {}
            Clause::And(l, r) => { l.hash(state); r.hash(state); }
            Clause::Or(l, r) => { l.hash(state); r.hash(state); }
            Clause::Xor(l, r) => { l.hash(state); r.hash(state); }
            Clause::Deontic(d) => d.hash(state),
            Clause::Dynamic(d) => d.hash(state),
        }
    }
}
...
pub fn build_ast(pair: Pair<Rule>) -> Result<Contract> {
    // ... (código existente)
    // ... (resto do arquivo ast_builder.rs sem alterações)
            ...
            Rule::conflict => {
                for conflict_part in inner_pair.into_inner() {
                    match conflict_part.as_rule() {
                        Rule::cfGlobal_block => {
                            let pairs = conflict_part.into_inner().filter(|p| p.as_rule() == Rule::cfPair);
                            for p in pairs {
...
fn build_clause_term(pair: Pair<Rule>) -> Result<Clause> {
    ...
    match inner.as_rule() {
        ...
        Rule::OPEN_EXP => {
            // Esta regra é anônima na gramática, 'pest' não a nomeia
            // mas ela contém 'clause'.
             let clause_pair = inner.into_inner().next().ok_or_else(|| AstError::BuildError("Esperava 'clause' dentro de '(...)'".to_string()))?;
...
fn build_alpha_atom(pair: Pair<Rule>) -> Result<Action> {
    ...
    match inner.as_rule() {
        ...
        Rule::OPEN_EXP => {
            let alpha_pair = inner.into_inner().next().ok_or_else(|| AstError::BuildError("Esperava 'alpha' dentro de '(...)'".to_string()))?;
            ...
            match pairs.next() {
                ...
                Some(op) => {
                    if op.as_rule() != Rule::OP_SEQ {
                         return Err(AstError::BuildError(format!("Token inesperado após ID em beta_term: {:?}, esperava '.' (OP_SEQ)", op.as_rule())));
                    }
                    // Regra: ID . beta
